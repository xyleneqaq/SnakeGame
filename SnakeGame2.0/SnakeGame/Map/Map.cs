...
public class Map
{
    ...
    public void MapElementSwitch(Map map,Vector vector,MapIndicator indicator)
    {
        map.MapCursor[vector] = indicator;
        if (indicator == MapIndicator.Free)
        {
            map.MapFlagCursor[vector] = MapFlag.Valid;
        }
        else
        {
            map.MapFlagCursor[vector] = MapFlag.Occupied;
        }
    }
}